...
import { ADMIN_AUTH_CODE } from "../admin/adminAuthcodeSafer.js";
import { AdminConfirm, AdminWarn, AdminUsers } from "../admin/adminSchemas.js";
...
router.get("/admin/online", async (req, res) => {
  if (
    ...
    req.session.user.authCode != undefined &&
    req.session.user.authCode === ADMIN_AUTH_CODE.get()
  ) {
    console.log("yup");
    ...
    AdminConfirm.findOne({ _id: 0 })
      .then((result) => {
        ...
        AdminWarn.findOne({ _id: 1 })
          .then((warn) => {
            AdminUsers.findOne({ _id: 2 }).then((users) => {
              redisClient
                .sMembers("logged_in_admins")
                .then((members) => {
                  redisClient.sMembers("selected").then((selecte) => {
