...
const handleWarn = async(req, res) => {    
    ...
    switch(req.body.filters){
        case "qna":
            ...
            if(r.warn>=10){
                const check = await mainInquiry.read(['warned'],req.body.decryptedSessionId);
                if(check.warned>8) mainInquiry.write({confirmed:1, warned:1},req.body.decryptedSessionId);
                else await mainInquiry.write({warned:1},req.body.decryptedSessionId);
